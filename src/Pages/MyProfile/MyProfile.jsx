...
const BuyerProfile = () => {
  ...
  const fetchProfile = async () => {
    ...
    const url = `${apiURL}/app/seller/fetch-seller/${token}`;
    ...
    if (res.data.message == "OK") {
      ...
      const updatedData = {
        ...
        BuyerProfilePicURL : Data.SellerProfilePicURL
      }
  ...
  const createBuyerAccount = async()=>{
    setLoading(true)
    const token = await Cookies.get('access_token');
    const res = await axios.post(`${apiURL}/app/seller/create-seller`,{
      token : token
    })
  ...
  loading ? (<LinearProgress />) :null
      ...
      {status!="OK" ? (
        <>
          ...
          >
            <Button onClick={createBuyerAccount} variant="contained">Create Seller Account</Button>
          </div>
        </>
      ) : buyer && (
        <>
